...
R peut à minima être utilisé comme une grosse calculatrice. Additions, soustractions, multiplication et divisions s'écrivent de la manière suivante (cliquer sur la flèches verte en haut à gauche de chaque cellule pour afficher le resultat :
...
R dispose de quelques jeu de données préinstallées et prêtes à être utilisées :
...
Pour en savoir plus sur ces jeux de données, vous pouvez utiliser la fonction **help** qui s'affiche dans un onglet séparé de Rstudio

```{r}
help("ToothGrowth")
```

Pour accéder aux jeux de données il sufit de taper leur nom. Mais si vous faites ça (allez-y !), R affiche par défaut tout le jeu de données (les 150 lignes). Dur de s'y retrouver. Voici quelques fonctions utiles pour prendre connaissance d'un jeu de données.

La fonction **dim** vous donne les dimensions du tableau. Ici 150 lignes et 5 colonnes
...
## Comment réaliser des opérations arithmétiques élémentaires sur ce tableau de données ?
...
Le symbole \$ sert à sélectionner une colonne d'un tableau. Par exemple si je souhaite récupérer la colonne "Sepal.Length" du dataset "iris", j'écrirai iris**\$**Species.
...
Il est ensuite possible de faire directement des opérations arityhmétiques sur ces vecteurs, comme par exemple :
